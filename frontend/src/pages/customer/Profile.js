  ...
  const handleProfileSave = async (e) => {
    ...
    try {
      ...
      formData.append('address', JSON.stringify({
        street: profileData.street, area: profileData.area,
        city: profileData.city, district: 'Rangpur'
      }));
      await updateProfile(formData);
      await fetchUser();
            ...
            <div className="profile-sidebar card">
              <div className="profile-avatar-wrapper">
                <div className="profile-avatar-big">
                  ...
                  {uploadingPhoto && <div className="avatar-uploading"><div className="spinner-sm" /></div>}
                </div>
                <button className="avatar-upload-btn" onClick={() => fileInputRef.current?.click()} disabled={uploadingPhoto}>
                  <FiCamera size={14} /> {uploadingPhoto ? 'Uploading...' : 'Change Photo'}
                </button>
                <input ref={fileInputRef} type="file" accept="image/jpg,image/jpeg,image/png,image/webp,image/gif" style={{ display: 'none' }} onChange={handleAvatarChange} />
              </div>
              <div className="profile-name">{user?.name}</div>
              <div className="profile-email">{user?.email}</div>
              <div className="profile-loyalty-badge"><FiUser size={14} /> {user?.loyaltyPoints || 0} pts</div>
            ...
            <div className="profile-content">
              ...
              {activeTab === 'profile' && (
                <form className="profile-form card" onSubmit={handleProfileSave}>
                  <div className="form-group"><label className="form-label"><FiUser size={14} /> Full Name</label><input className="form-input" value={profileData.name} onChange={e => setProfileData({ ...profileData, name: e.target.value })} /></div>
                  <div className="form-group"><label className="form-label"><FiPhone size={14} /> Phone</label><input className="form-input" value={profileData.phone} onChange={e => setProfileData({ ...profileData, phone: e.target.value })} /></div>
              ...
              {activeTab === 'password' && (
                <form className="profile-form card" onSubmit={handlePasswordSave}>
                  <div className="form-group"><label className="form-label"><FiLock size={14} /> Current Password</label><input type="password" className="form-input" value={passwordData.currentPassword} onChange={e => setPasswordData({ ...passwordData, currentPassword: e.target.value })} /></div>
              ...
              {activeTab === 'account' && (
                <div className="profile-form card">
                  <div className="delete-account-section">
                    ...
                    <p>Once you delete your account, all your data including order history and loyalty points will be permanently removed. This action <strong>cannot be undone</strong>.</p>
      ...
      {showDeleteConfirm && (
        <div className="confirm-overlay" onClick={() => setShowDeleteConfirm(false)}>
          <div className="confirm-box" onClick={e => e.stopPropagation()}>
            ...
            <p>Are you sure you want to permanently delete your account? All your data, orders, and loyalty points will be lost forever.</p>
